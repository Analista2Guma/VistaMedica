...
public class TaskDeleteFarmacias extends AsyncTask<String,Integer,String> {
    Context cnxt;

    private static final String TAG = "TaskDeleteFarmacias";
    public TaskDeleteFarmacias(Context cnxt) {
        this.cnxt = cnxt;
    ...
    protected String doInBackground(final String... params) {
       ...
       Class_retrofit.Objfit().create(Servicio.class).Send_Id_delete_Row(params[0]).enqueue(new Callback<String>() {
            @Override
            public void onResponse(Call<String> call, Response<String> response) {
                if(response.isSuccessful()){

                    if (response.body().equals("OK")){
                        Farmacias_model.Delete(ManagerURI.getDirDb(),cnxt,params[0]);
